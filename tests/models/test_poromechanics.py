...
@pytest.mark.parametrize("model", [TailoredPoromechanics, TailoredPoromechanicsTpsa])
def test_2d_single_fracture(solid_vals, north_displacement, model):
...
@pytest.mark.parametrize("model", [TailoredPoromechanics, TailoredPoromechanicsTpsa])
def test_pull_north_positive_opening(model):
    ...
    # Tpsa; for reference, the respective value for Mpsa was of the order of 1e-7. Why
    # the accuracy is lower with tpsa is not clear to me, but it should be mentioned at
    # the time of writing, tpsa has not been thoroughly tested on problems with
    # fractures.
...
@pytest.mark.parametrize("model", [TailoredPoromechanics, TailoredPoromechanicsTpsa])
def test_pull_south_positive_opening(model):
...
def test_positive_p_frac_positive_opening():
    ...
    # EK note to posterity: When introducing tpsa, I had to changes this from 4.7e-4 to
    # 4.2e-4. I have no idea why tpsa produces such a different result (~10% difference),
    # in a sense it is what it is.
    assert np.all(p_frac > 4.2e-4)
...
@pytest.mark.parametrize("model", [TailoredPoromechanics, TailoredPoromechanicsTpsa])
def test_unit_conversion(units, model):

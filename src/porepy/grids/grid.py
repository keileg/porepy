...
class Grid:
    ...
    def cell_diameters(
        ...
        Parameters:
            ...
            cell_wise:
                If True, returns the cell diameters. If False, returns the maximum
                diameter of all cells.
        ...
        if cell_wise:
            ...
            # the below else, but EK does not have the time to do so now.
            ...
            # maximum diameter of all cells. The idea is to get a unique combination of
            # node pairs (thus considering only once those cases where two cells share a
            # node pair, for instance along an edge), and then compute the diameter of
            # only those node pairs. It should be possible to give a cell-wise diameter
            # by using this technique, but that would require a mapping from the unique
            # node pairs back to the cells (which could be trivial if all cells have
            # equally many nodes, but will be more complex for general cases). This has
            # not been prioritized, instead the cell-wise diameter is computed in the
            # above if.
